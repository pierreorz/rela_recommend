...
func GetLiveFeaturesV2(ctx algo.IContext, model algo.IAlgo, idata algo.IDataInfo) *utils.Features {
	...
	userCache := userInfo.UserCache
	liveCache := liveInfo.LiveCache
	liveUserCache := liveInfo.UserCache
	// 用户连续特征 1 - 500
	fs.Add(1, float32(userCache.Age))
	fs.Add(2, float32(userCache.Height))
	fs.Add(3, float32(userCache.Weight))
	...
	// 	// 用户观看行为embedding
	// 	fs.AddArray(100, 30, userInfo.LiveProfile.LiveViewUserEmbedding)
	// }
	...
	if liveUserCache != nil {
		...
		fs.Add(502, float32(liveUserCache.Height))
		fs.Add(503, float32(liveUserCache.Weight))
		fs.Add(504, float32(ctx.GetCreateTime().Sub(liveUserCache.CreateTime.Time).Seconds() / 60 / 60 / 24))			// 直播注册时长 day
	}
	fs.Add(505, float32(liveCache.FansCount))		// 粉丝数量
	fs.Add(506, liveCache.DayIncoming)				// 天收入
	fs.Add(507, liveCache.MonthIncoming)			// 月收入
	...
	fs.AddCategory(10200, 5, 0, ctx.GetPlatform(), 0)	// 用户操作系统
	fs.AddCategory(10205, 5, 0, rutils.GetPlatform(liveCache.Live.Ua), 0)	// 主播操作系统

	fs.AddCategory(10210, 10, 0, liveCache.Live.AudioType, 0)	// 房间类型
	fs.AddCategory(10220, 10, 0, liveCache.Live.IsMulti, 0)		// 房间是否多人
	if userInfo.UserConcerns != nil {  // 用户是否是主播粉丝
		fs.AddCategory(10230, 2, 0, rutils.GetInt(userInfo.UserConcerns.Contains(liveCache.Live.UserId)), 0)
	}	
	fs.AddCategory(10232, 3, -1, liveCache.Recommand, 0)		// 主播是否被推荐(-1, 0, 1)

	// 交叉特征    15000 - 20000
	if liveInfo.LiveProfile != nil && userInfo.LiveProfile != nil {	// 隐含特征推荐度
		recVal := utils.ArrayMultSum(userInfo.LiveProfile.LiveViewUserEmbedding, liveInfo.LiveProfile.LiveViewLiveEmbedding)
		fs.Add(15000, recVal)
	}
