...
func TestRouter(t *testing.T) {
	router := New("127.0.0.1:81")

	routed := false
	router.Handle("GET", "/user/:name", func(pContext *Context) {
		routed = true
		want := Params{Param{"name", "gopher"}}
		if !reflect.DeepEqual(pContext.Params, want) {
			t.Fatalf("wrong wildcard values: want %v, got %v", want, pContext.Params)
		}
	})
...
func TestRouterAPI(t *testing.T) {
	...
	router := New("127.0.0.1:81")
	router.GET("/GET", func(pContext *Context) {
		get = true
	})
	router.HEAD("/GET", func(pContext *Context) {
		head = true
	})
	router.OPTIONS("/GET", func(pContext *Context) {
		options = true
	})
	router.POST("/POST", func(pContext *Context) {
		post = true
	})
	router.PUT("/PUT", func(pContext *Context) {
		put = true
	})
	router.PATCH("/PATCH", func(pContext *Context) {
		patch = true
	})
	router.DELETE("/DELETE", func(pContext *Context) {
		delete = true
	})
...
func TestRouterChaining(t *testing.T) {
	router1 := New("127.0.0.1:81")

	fooHit := false
	router1.POST("/foo", func(pContext *Context) {
		fooHit = true
		pContext.Writer.WriteHeader(http.StatusOK)
	})
...
func TestRouterOPTIONS(t *testing.T) {
	handlerFunc := func(pContext *Context) {}
	...
	// custom handler
	var custom bool
	router.OPTIONS("/path", func(pContext *Context) {
		custom = true
	})
...
func TestRouterNotFound(t *testing.T) {
	...
	var notFound bool
	router.NotFound = func(pContext *Context) {
		pContext.Writer.WriteHeader(404)
		notFound = true
	}
...
func TestRouterPanicHandler(t *testing.T) {
	...
	router.PanicHandler = func(pContext *Context, p interface{}) {
		panicHandled = true
	}

	router.Handle("PUT", "/user/:name", func(pContext *Context) {
		panic("oops!")
	})
...
func TestRouterLookup(t *testing.T) {
	routed := false
	wantHandle := func(pContext *Context) {
		routed = true
	}
