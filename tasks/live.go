...
var cachedLiveListMap = map[int][]pika.LiveCache{}
var cachedLiveListMapUpdateLocker = &sync.RWMutex{}

// GetCachedLiveMapList 获取缓存的直播列表 -1.all; 1. video; 2. audio; 3. multi_audio(radio)
...
// GetCachedLiveMap 获取缓存的直播用户id为key 的 map
func GetCachedLiveMap(liveType int) map[int64]*pika.LiveCache {
...
// GetCachedLiveListByTypeClassify 通过直播分类获取直播列表
func GetCachedLiveListByTypeClassify(typeId int, classify int) []pika.LiveCache {
	// 客户端和服务端约定了两个大数分别代表 video 和 multi_audio
	if classify == api.TypeBigVideo {
		typeId = 1
		classify = api.TypeRecommend
	} else if classify == api.TypeBigMultiAudio {
		typeId = 3
		classify = api.TypeRecommend
	}
	...
	for i, live := range liveList {
		if classify == api.TypeGroupVideo {
			// 二十大会特殊审核逻辑，把叽叽喳喳单独列出来
			if live.IsGroupVideo {
				lives = append(lives, liveList[i])
			}
		} else if classify <= api.TypeRecommend || live.Live.Classify == classify {
...
func convertApiLive2RedisLiveList(lives []api.SimpleChatroom) []pika.LiveCache {
	...
	for i, live := range lives {
		...
		var item api.ILiveRankItemV3
		err := json.Unmarshal([]byte(live.Data), &item)
		if err == nil {
			liveCache.IsGroupVideo = item.IsGroupVideo()
		}
	}
...
func refreshLiveMapList(duration time.Duration) {
	...
	for {
		...
		select {
		case <-tick.C:
			pf.Run("tasks.live", func(perform *performs.Performs) interface{} {
		...
		_ = pf.ToWriteChan("algo.task", map[string]string{
			"app": "live.rpc",
...
// 启动自动刷新直播列表缓存
func init() {
	go refreshLiveMapList(15 * time.Second)
}
